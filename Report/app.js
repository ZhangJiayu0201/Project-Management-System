...
function loadUsers() {
  var saved = localStorage.getItem("pmsUsers");
  if (saved) {
...
function saveUsers() {
  localStorage.setItem("pmsUsers", JSON.stringify(users));
}
...
function login() {
  ...
  for (var i = 0; i < users.length; i++) {
    if (users[i].name === name && users[i].pass === pass) {
      localStorage.setItem("pmsCurrentUser", name);
...
function registerUser() {
  ...
  if (name === "" || pass === "") {
    document.getElementById("regMsg").innerHTML = "Please enter username and password";
    return false;
  }
  ...
  users.push({ name: name, pass: pass, role: role });
  saveUsers();
  document.getElementById("regMsg").innerHTML = "Register success";
...
function logout() {
  localStorage.removeItem("pmsCurrentUser");
...
function checkLoginForApp() {
  ...
  currentUser = localStorage.getItem("pmsCurrentUser");
  if (!currentUser) {
    window.location.href = "index.html";
    return;
  }

  document.getElementById("currentUserText").innerHTML = "Current user: " + currentUser;
}

function taskKey() {
  return "pmsTasks_" + currentUser;
}
...
function loadTasks() {
  ...
  if (saved) {
    ...
    tasks = [
      ...
      { id: 2, title: "Create assignment UI", member: "Manager", done: false, deadline: "2026-08-20", update: "Working" },
      { id: 3, title: "Add update field", member: "Worker", done: false, deadline: "2026-06-01", update: "Working" }
    ];
    saveTasks();
  }
}
  ...
  for (var i = 0; i < tasks.length; i++) {
    if (member === "All" || tasks[i].member === member) {
      ...
      row.innerHTML =
        ...
        "<td><select id='done" + tasks[i].id + "'><option value='false'>Not Done</option><option value='true'>Done</option></select></td>" +
        "<td>" + tasks[i].deadline + "</td>" +
        "<td><input id='update" + tasks[i].id + "' value='" + tasks[i].update + "' placeholder='task update'></td>" +
...
function addTask() {
  ...
  tasks.push({ id: id, title: title, member: member, done: false, deadline: deadline, update: "New task" });
  saveTasks();
  showTasks();
  setMsg("Task added");
  return true;
...
function setMsg(text) {
  var msg = document.getElementById("appMsg");
  if (msg) {
    msg.innerHTML = text;
  }
}
...
function makeTestStore() {
  return {
    tasks: [
      ...
      { id: 2, title: "Task 2", member: "Manager", done: false, deadline: "2026-08-20", update: "Start" },
...
TestApp.prototype.registerUser = function(name, pass) {
  if (name === "" || pass === "") {
    return false;
  }
  this.store.users.push({ name: name, pass: pass, role: "Student" });
  return true;
};
...
function runTests() {
  ...
  var t14 = new TestApp(makeTestStore());
  check("Register user", t14.registerUser("newuser", "123"));
...
function checkLoginForSignIn() {
  if (!document.getElementById("loginName")) {
    return;
  }

  var signedUser = localStorage.getItem("pmsCurrentUser");
